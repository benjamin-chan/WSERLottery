...
#---------------------
distn <- c(1727, 561, 258, 106, 52)  # Number of entrants for each ticket count
probWSER <- c(6.5, 12.6, 18.3, 23.6, 28.5)  # Selection probabilities from WSER
spots <- 270  # Number of spots up for grabs
size <- 1E3  # Use 1E6 for production, 1E3 for testing
...
confail <- factor(rep(seq(1, 5), size))
freq <- as.vector(t(aggLottery[, list(confail.0, confail.1, confail.2, confail.3, confail.4)]))
...
y1 <- max(density(frameSummary$prob[frameSummary$confail == 0])$y)
y2 <- max(density(frameSummary$prob[frameSummary$confail == 1])$y)
y3 <- max(density(frameSummary$prob[frameSummary$confail == 2])$y)
y4 <- max(density(frameSummary$prob[frameSummary$confail == 3])$y)
y5 <- max(density(frameSummary$prob[frameSummary$confail == 4])$y)
y <- c(y1, y2, y3, y4, y5)
...
y1 <- max(density(frameSummary$freq[frameSummary$confail == 0])$y)
y2 <- max(density(frameSummary$freq[frameSummary$confail == 1])$y)
y3 <- max(density(frameSummary$freq[frameSummary$confail == 2])$y)
y4 <- max(density(frameSummary$freq[frameSummary$confail == 3])$y)
y5 <- max(density(frameSummary$freq[frameSummary$confail == 4])$y)
y <- c(y1, y2, y3, y4, y5)
