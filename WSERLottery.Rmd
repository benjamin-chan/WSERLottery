...
distn <- c(1427, 641, 281, 136, 57, 24)  # Number of entrants for each ticket count
freq <- distn / sum(distn)  # Frequency distribution from previous year
freq <- floor(freq * 1e3) / 1e3  # Round down to the nearest 1e-3
freq <- c(freq, 1 - sum(freq))  # Add the remainder to the newly greatest ticket level
nEntrants <- 3563  # as of 2015-11-15 (https://ultrasignup.com/lottery/lottery_applicants.aspx?did=34773)
distn <- round(nEntrants * freq)
probWSER <- c(4.7, 9.3, 17.6, 32.2, 54.0, 78.8, 0)  # Selection probabilities from WSER
