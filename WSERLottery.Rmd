...
sqerr <- diffprob ^2
simsum <- data.frame(avg, med[, 2], sd[, 2], distn, ev, probWSER, evWSER, diffprob, diffev, pctdiff, sqerr)
names(simsum) <- c("Tickets", "Mean", "Median", "SD", "N", "EV", "Prob (WSER)", "EV (WSER)", "Diff. prob.", "Diff. EV", "% diff.", "Sq. error")
```
...
```
My estimates and the probabilities calculated by WSER are essentially identical. Percent differences of the selection probabilities are never more than `r max(abs(simsum$"% diff."))`% and the mean squared error of the selection probabilities is `r sprintf("%.6f", mean(simsum$"Sq. error"))`.
