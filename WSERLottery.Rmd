...
aggTime <- system.time(
  aggLottery <- frameLottery[,
                             list(tickets.1 = sum(tickets == 1),
                                  tickets.2 = sum(tickets == 2),
                                  tickets.3 = sum(tickets == 3),
                                  tickets.4 = sum(tickets == 4),
                                  tickets.5 = sum(tickets == 5)),
                             sim]
)
sim <- rep(seq(1, size), each=5)
tickets <- factor(rep(seq(1, 5), size))
freq <- as.vector(t(aggLottery[, list(tickets.1, tickets.2, tickets.3, tickets.4, tickets.5)]))
frameSummary <- data.frame(sim, tickets, freq)
require(reshape2)
frameSummary <- melt(aggLottery, id.vars=c("sim"))
frameSummary$tickets <- factor(gsub("tickets\\.", "", frameSummary$variable))
frameSummary$freq <- frameSummary$value
frameSummary[, c("variable", "value")] <- NULL
frameSummary <- frameSummary[order(frameSummary$sim, frameSummary$tickets)]
# 
# Format lottery simulation data
# ------------------------------
